"""
    This file contains required classes/functions to aggregate data from the various
    input formats into a single output frame and a set of coordinates to localize
    the overlay with respect to either the global image, or the centerpoint of the
    main PCB contour

    Pseudo-code//brainstorm
    - Accept as input any number of features from the gerbers or design file
    - Produce some sort of sprite for each graphical element
    - Fit the elements onto the required output frame, avoiding collision between elements
        - for each element, calcualte some x-y offset from a point of reference that all filetypes
        can utilise accurately
    - Generate the overlay with transparency via opencv2 drawing functions, that way the object type
    is directly compatable with the end-point drawing stack ( We'll combine the overlay and camera stream in opencv )

    Structure
    - Format this file as a one-off short-lived function

    TODO:
    - Determine the various input formats before continuing
"""
...
import cv2
import numpy as np
from kicad_pcbnew_interface import test_output
...
if __name__ == '__main__':
    ...
    frame = cv2.imread("/home/matt/Desktop/projects/pcb_overlay/assets/overlay_test/Ganglion-FRONT-2.jpg")
    backup = frame.copy()
    # Load overlay image
    file_output = test_output()
    overlay = cv2.imread('/home/matt/Desktop/projects/gerber_cli/src/output.png')
    overlay = cv2.resize(overlay, (0,0), fx=3.3, fy=3.3)
